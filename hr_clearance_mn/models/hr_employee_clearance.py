...
class HrEmployeeClearance(models.Model):
    ...
    _description = "Employee Clearance Form"
    ...
    it_check_done = fields.Boolean(string="IT / хандалт шалгасан", tracking=True)
    it_check_date = fields.Date(string="IT огноо", tracking=True)
    it_note = fields.Text(string="IT тэмдэглэл")
    ...
    final_hr_done = fields.Boolean(string="Эцсийн HR баталгаажуулалт", tracking=True)
    final_hr_date = fields.Date(string="Эцсийн HR огноо", tracking=True)
    ...
    def _get_section_completion_messages(self, vals):
        section_labels = {
            ...
            "it_check_done": "IT / хандалтын шалгалт дууслаа.",
            "asset_check_done": "Эд хөрөнгийн шалгалт дууслаа.",
            "manager_check_done": "Удирдлагын шалгалт дууслаа.",
            "final_hr_done": "Эцсийн HR баталгаажуулалт хийгдлээ.",
        }
        completion_messages = {}
        for record in self:
            messages = []
    ...
    def _validate_done_requirements(self):
        for record in self:
            ...
            if not record.final_hr_done:
                missing.append(_("Эцсийн HR баталгаажуулалт"))
            if missing:
